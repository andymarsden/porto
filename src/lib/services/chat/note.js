...
export function createNote(content) {
    const defaultTitle = deriveDefaultTitle(content);
...
export function addTitleToNote(noteId, title) {
    ...
    note.isAutoGeneratedTitle = false;
...
export function loadNotes(rawJson) {
    try {
        ...
        notes.length = 0;
        parsed.forEach((n) => {
            const hasAutoTitleFlag = typeof n?.isAutoGeneratedTitle === "boolean";
            const inferredAutoTitle =
                typeof n?.title === "string" && n.title === deriveDefaultTitle(n?.content);

            notes.push({
                ...n,
                isAutoGeneratedTitle: hasAutoTitleFlag ? n.isAutoGeneratedTitle : inferredAutoTitle
            });
        });
    } catch {
